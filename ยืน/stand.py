...
def process_video():
    ...
    width = 1240
    height = 670
    pillar_height_cm = 176.3
    
    start_time = time.time()  # เวลาเริ่มต้นการจับเวลา
    while True:
        ...
        if indices is not None and len(indices) > 0:
            for i in indices.flatten():
                ...
                cv2.putText(frame, height_text, (x, y+h+20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                
                elapsed_time = time.time() - start_time  # คำนวณเวลาที่ผ่านไป
                if elapsed_time >= 10:  # ถ้าผ่านไปเวลา 10 วินาที
                    cv2.imwrite("captured_frame.jpg", frame)  # ถ่ายภาพเฟรม
                    print("Frame captured and saved.")
                    start_time = time.time()  # เริ่มต้นการจับเวลาใหม่
